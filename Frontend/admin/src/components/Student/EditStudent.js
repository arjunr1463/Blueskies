...
function EditStudent({ click }) {
  ...
  const [contactNumber, setContactNumber] = useState(location.state.mobile);
  const [email, setEmail] = useState(location.state.email);
  const [qualification, setQualification] = useState(
    location.state.qualification
  );
  const [Certified, setCertified] = useState(location.state.certified);
  const [address, setAddress] = useState(location.state.address);
  const [courseFee, setCourseFee] = useState(location.state.coursefee);
  const [paymentType, setPaymentType] = useState(location.state.paymenttype);
  const [courseStartDate, setCourseStartDate] = useState(
    location.state.coursestart
  );
  const [courseEndDate, setCourseEndDate] = useState(location.state.courseend);
  const [studentPhoto, setStudentPhoto] = useState(null);
  const [imageName, setImageName] = useState(location.state.image.contentType);
  ...
  const handleStudentPhotoChange = (event) => {
    ...
    if (file) {
      ...
      setStudentPhoto(location.state.image.data.data);
  ...
  const handleSubmit = () => {
    ...
    try {
      axios
        .put(
          `${process.env.REACT_APP_API_URL}/api/user/admin/edit/${id}`,
          formData
        )
        .then((res) => {
          setSuccess(res.data);
